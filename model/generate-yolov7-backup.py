...
def convert_txt(json_path,marks_content, full_w,full_h):
    ...
    def parse_item(item,mark):
        ...
        if "width" in item: # then this is rectangular
            ...
            w = 2 * item['outerRadius']
            h = 2 * item['outerRadius']
            category = "circular"
        else:
            return None
        cx /= full_w
        ...
        full_classname = f"{category}-{mark}"
        print(full_classname)
        ind = get_class_ind(full_classname)
        if ind == -1:
            return None
        return [f"{ind} {rest_data}"]
    print(marks_content)
    print(json_path)
    print(data)
    print(len(data),len(marks_content))
