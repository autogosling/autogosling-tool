...
cuda = True
w = "/new_mem/manqing/archive/best_jan.onnx" # Janurary archive
# w = "./best.onnx"
# w = "./best_backup.onnx"
...
def letterbox(im, new_shape=(640, 640), color=(0, 0, 0), auto=True, scaleup=True, stride=32):
    '''
    Input: an image that will be fed to YoloV7.
    Output: an image that has been properly padded and resized that YoloV7 can understand
    '''
...
def multihot_element(label,nc):
    '''
    Input: an integer that stands for multiple labels
    Output: the deconstructed multiple labels in a list
    e.g. label=25,nc=6 -> 2^4 + 2^3 + 2^0 = [1 0 0 1 1 0]
    '''
...
def predict(img,is_multihot=False):
    '''
    Input: An arbitrary image to be fed to YoloV7
    Output: annotated images with labels and scores
    '''
    ...
    def make_image(class_set):
        ...
        for i,(batch_id,x0,y0,x1,y1,cls_id,score) in enumerate(outputs):
            ...
            if is_multihot:
                ...
                arr = multihot_element(cls_id,NC)
                layout = arr[0]
                orientation = arr[1]
                marks = [i for i, el in enumerate(arr[2:]) if el == 1]
                multiple_classes =  [marks,layout,orientation]
...
def draw_bounding_boxes(im: PIL.Image, bboxes: np.ndarray, classes: np.ndarray,
    ...
    '''
    Input: a PIL image and a list of bounding boxes and corresponding classes + scores
    Output: an annotated PIL image with the bounding boxes and labels on them
    '''
    im = im.copy()
